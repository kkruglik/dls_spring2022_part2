...
def train(model, iterator, optimizer, scheduler, criterion, clip, train_history=None, valid_history=None):
    ...
    for i, batch in tqdm(enumerate(iterator)):
        ...
        loss = criterion(preds, batch['outputs'])
        loss.backward()
...
def evaluate(model, iterator, criterion):
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(iterator)):
            preds = model(batch['inputs'])
            loss = criterion(preds.softmax(dim=1), batch['outputs'])
            epoch_loss += loss.item()
    return epoch_loss / len(iterator)
...
def train_with_logs(model, train_iterator, valid_iterator, optimizer, scheduler, criterion, N_EPOCHS, CLIP):
    train_history = []
    valid_history = []
